...
contract UGArena is IUGArena, Ownable, ReentrancyGuard, Pausable {
  ...
  constructor(address _ugnft, address _ugFYakuza, address _blood, address _randomizer) {
    ugNFT = IUGNFT(_ugnft);
  ...
  function getStakedFighterIDsForUser(address user) public view returns (uint256[] memory){
    ...
    if(numStakedFighters == 0) return new uint256[](0);
    uint256[] memory _tokenIds = stakedByOwner(user);
    uint256[] memory packedFighters = ugFYakuza.getPackedFighters(_tokenIds);
    uint256[] memory stakedFighterIds = new uint256[](numStakedFighters);
    //loop through user balances until we find all the fighters
    uint count;
    uint ttlFYs = _tokenIds.length;
    for(uint i; count<numStakedFighters && i < ttlFYs; i++){
      if(unPackFighter(packedFighters[i]).isFighter){       
        stakedFighterIds[count] = _tokenIds[i];
        count++;          
      }
    }
    return stakedFighterIds;
  }

  function getStakedYakuzaIDsForUser(address user) public view returns (uint256[] memory){
    //get balance of fighters
    uint256 numStakedYakuzas = getValueInBin(userTotalBalances[user], USER_TOTAL_BALANCES_BITS_SIZE, YAKUZA_INDEX);
    if(numStakedYakuzas == 0) return new uint256[](0);
    uint256[] memory _tokenIds = stakedByOwner(user);
    uint256[] memory packedFighters = ugFYakuza.getPackedFighters(_tokenIds);
    uint256[] memory stakedYakuzaIds = new uint256[](numStakedYakuzas);
    //loop through user balances until we find all the fighters
    uint count;
    uint ttlFYs = _tokenIds.length;
    for(uint i; count<numStakedYakuzas && i < ttlFYs; i++){
      if(!unPackFighter(packedFighters[i]).isFighter){       
        stakedYakuzaIds[count] = _tokenIds[i];
        count++;          
      }
    }
    return stakedYakuzaIds;
  }
  ...
  function stakedByOwner(address owner) public view returns (uint256[] memory) {
    uint256 ownerTokenCount = getValueInBin(userTotalBalances[owner], USER_TOTAL_BALANCES_BITS_SIZE, FIGHTER_INDEX) +
      getValueInBin(userTotalBalances[owner], USER_TOTAL_BALANCES_BITS_SIZE, YAKUZA_INDEX);
 ...
 function calculateAllStakingRewards(uint256[] memory tokenIds) external view returns (uint256 owed) {
  ...
  function _calculateStakingRewards(
    ...
    if (fighter.isFighter) { // Fighter
      ...
      if(_bloodPerRank  > myStake.bloodPerRank) owed = (fighter.rank) * (_bloodPerRank - myStake.bloodPerRank) / 100000;
    }
    return (owed);
  }
  ...
  function _payYakuzaTax(uint amount) private {
    ...
    _bloodPerRank += 100000 * (amount + _unaccountedRewards) / totalRankStaked;
  ...
  function claimManyFromArena(uint256[] calldata tokenIds, bool unstake) external whenNotPaused nonReentrant {
    ...
    if(account == address(0)) account = getStakedYakuza(tokenIds[0]).owner;
  ...
  function _claimYakuza(uint256 tokenId, bool unstake, IUGFYakuza.FighterYakuza memory fighter) private returns (uint256 owed) { 
    Stake memory stake = _getStake(tokenId);
  ...
  */
  ...
  {    
    ...
    uint newIndex = getValueInBin(userTotalBalances[_address], USER_TOTAL_BALANCES_BITS_SIZE, FIGHTER_INDEX) +
      getValueInBin(userTotalBalances[_address], USER_TOTAL_BALANCES_BITS_SIZE, YAKUZA_INDEX);
    uint bin;
    uint index;
    for(uint i; i< _tokenIds.length;i++){
  ...
  function _removeFromOwnerStakedTokenList(address _from, uint256[] memory _tokenIds)internal{    
    uint256 lastTokenIndex = getValueInBin(userTotalBalances[_from], USER_TOTAL_BALANCES_BITS_SIZE, FIGHTER_INDEX) +
      getValueInBin(userTotalBalances[_from], USER_TOTAL_BALANCES_BITS_SIZE, YAKUZA_INDEX) - 1;
    //16 tokenIds per uint
    uint256 bin;
    uint256 index;
  ...
  function tokenOfOwnerByIndex(address owner, uint256 _index) public view virtual returns (uint256) {
    require(_index < getValueInBin(userTotalBalances[owner], USER_TOTAL_BALANCES_BITS_SIZE, FIGHTER_INDEX) +
      getValueInBin(userTotalBalances[owner], USER_TOTAL_BALANCES_BITS_SIZE, YAKUZA_INDEX), "owner index out of bounds");
